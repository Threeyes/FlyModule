...
public class CreatureFlightPhysics : FreeFlightPhysics
{
    ...
    public void accelerateInput(float forward, float sensitivity)
    {
        //仅当前速度小于最大速度时，才可进行加速
        if (Speed < MaxSpeed)
            rigidbody.AddForce(rigidbody.rotation * Vector3.forward * forward * sensitivity);
    }
    ...
    public void directionalInput(float bank, float pitch, float sensitivity)
    {
        ...
        rigidbody.MoveRotation(Quaternion.Lerp(rigidbody.rotation, _desiredDirectionalInput, sensitivity * Time.fixedDeltaTime));
        //Debug.LogError($"directionalInput: [{bank}] [{pitch}] [{sensitivity * Time.fixedDeltaTime}]");
    }
    ...
    {
        currentFlapTime += Time.fixedDeltaTime;
    ...
    /// 
    /// ToTest:
    /// -根据左右翼传入的数值不同，可以实现左右偏转。如果左右翼传入相同，则不偏转
    /// </summary>
    ...
    public void wingFold(float left, float right)
    {
        ...
        rigidbody.AddTorque(rigidbody.rotation * Vector3.forward * (right - left) * torqueSpeed);//增加旋转力
        ...
        if (!wingsOpen())
        {
            ...
            pitchRot.eulerAngles = new Vector3(AngleOfAttack, 0, 0);//仅保留X轴的偏转
            pitchRot = rigidbody.rotation * pitchRot;
            rigidbody.rotation = Quaternion.Lerp(rigidbody.rotation, pitchRot, torqueSpeed * Time.fixedDeltaTime * 5);//根据速度，偏移旋转
